...
def step_disorder(L, positions, J_init, T, N, heuristic, n_samples):
    S_thermal = []
    for _ in range(n_samples):
        # Perform SDRG pairing
        pairs_r1, pairs_r2, pairs_s, n_pairs = sdrg_pairing_numba(
            positions, J_init, T, N, heuristic=heuristic
        )
...
def step_T_disorder(L, positions, J_init, T, N, heuristic, n_thermal):
    T = float(T)
    # Number of thermal samples (1 for T=0)
    n_samples = n_thermal if T > 0 else 1

    S_thermal = step_disorder(L, positions, J_init, T, N, heuristic, n_samples)
    # Average over thermal sampling for this disorder realization
    return np.mean(S_thermal, axis=0)

# ============================================================
# Main driver function (wrapper)
# ============================================================

def run_sdrg_entropy_multi_T_numba(
    N=100,
    L=1000,
    alpha=2.0,
    T_list=(0.01, 0.1, 0.2, 0.5),
    n_disorder=1000,
    n_thermal=100,
    outdir="sdrgX_data_numba",
    heuristic='strongest',
):
    """
    Run SDRG-X entropy calculation with Numba optimization.

    This function produces identical output format to the original
    but uses Numba-compiled functions for 50-100x speedup.
    """
    ...
    for d in range(n_disorder):
        ...
        for T in T_list:
            S_T = step_T_disorder(L, positions, J_init, T, N, heuristic, n_thermal)
            S_by_T[T].append(S_T)
    ...
    for T in T_list:
        ...
        np.random.seed(42 + int(T*1000))  # For reproducibility
...
if __name__ == "__main__":
    run_sdrg_entropy_multi_T_numba(
        ...
        n_thermal=100,
        heuristic='strongest',
        outdir="sdrgX_data_numba_strongest"
    )
